...
namespace ShapeUp.Core.ShapeEditor
{
    public partial class SegmentGenerator
    {
        ...
        [SerializeField]
        public bool bezierQuadratic = false;
        ...
        private void Bezier_DrawSegments()
        {
            ...
            if (!bezierQuadratic && bezierGridSnapSize == 0f && segment.shape.symmetryAxes == SimpleGlobalAxis.None)
            ...
            {
                GL.Color(Color.cyan);
                GLUtilities.DrawDottedLine(1.0f, p1, p2);
                GLUtilities.DrawDottedLine(1.0f, p2, p4);
            }
            else
            {
                GL.Color(Color.blue);
        ...
        private IEnumerable<float2> Bezier_ForEachSegmentPoint()
        {
            ...
            // calculate quadratic curves using bezier curves.
            if (bezierQuadratic)
                MathEx.GetBezierControlPointsForQuadraticCurve(p1, p2, p4, out p2, out p3);
