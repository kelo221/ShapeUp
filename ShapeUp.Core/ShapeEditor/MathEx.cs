...
namespace ShapeUp.Core.ShapeEditor
{
    public static class MathEx
    {
        ...
        public static float2 Snap(this float2 value, float snap) =>
            new float2(Snapping.Snap(value.x, snap), Snapping.Snap(value.y, snap));
        ...
        /// <summary>Shortest distance from a point to a finite segment (for editor hit-testing).</summary>
        public static float DistanceToSegment(float2 point, float2 a, float2 b)
        {
            var nearest = FindNearestPointOnLine(point, a, b);
            return math.distance(point, nearest);
        }
        ...
        public static float2 FindNearestPointOnLine(float2 point, float2 origin, float2 end)
        {
            ...
            if (magnitudeMax < EPSILON_5) return origin;
            var dir = heading / magnitudeMax;
            var lhs = point - origin;
            var dotP = lhs.x * dir.x + lhs.y * dir.y;
