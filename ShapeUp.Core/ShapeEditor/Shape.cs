...
namespace ShapeUp.Core.ShapeEditor
{
    ...
    public class Shape
    {
        ...
        public Shape()
        {
            // Fixed ±1 world unit footprint → 2×2 units → 128×128 Quake map units at 64 units/world (TrenchBroom export).
            ResetToBox();
        }

        /// <summary>Half-edge length for a ~2×2 world-unit square with corners on multiples of <paramref name="snapIncrement"/> (optional; not used for default).</summary>
        public static float DefaultBoxHalfExtent(float snapIncrement)
        {
            if (snapIncrement < 1e-8f)
                return 1f;
            var n = Math.Max(1, (int)Math.Round(1.0 / snapIncrement));
            return n * snapIncrement;
        }

        /// <summary>Axis-aligned square centered on origin; corners snap to the given grid step.</summary>
        public void ResetToBoxSnapped(float snapIncrement)
        {
            ResetToSquare(DefaultBoxHalfExtent(snapIncrement));
        }

        /// <summary>Resets the shape to an axis-aligned square with half-extent <paramref name="halfExtent"/> (corners at ±halfExtent).</summary>
        public void ResetToSquare(float halfExtent)
        {
            segments.Clear();
            var h = halfExtent;
            AddSegment(new Segment(this, -h, -h));
            AddSegment(new Segment(this, h, -h));
            AddSegment(new Segment(this, h, h));
            AddSegment(new Segment(this, -h, h));
        }

        /// <summary>2×2 world-unit square (±1 corners). TrenchBroom clipboard uses 64 map units per world unit → 128×128 horizontal span.</summary>
        public void ResetToBox() => ResetToSquare(1f);
