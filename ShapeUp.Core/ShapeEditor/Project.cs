...
namespace ShapeUp.Core.ShapeEditor
{
    ...
    public class Project
    {
        ...
        /// <summary>True if some segment has both endpoints selected (one or more whole edges).</summary>
        public bool HasAnyFullySelectedEdge()
        {
            Validate();
        ...
        public Segment FindSegmentLineAtPosition(float2 position, float maxDistance)
        {
            Segment result = null;
            ...
            for (int i = 0; i < shapesCount; i++)
            {
                ...
                for (int j = 0; j < segmentsCount; j++)
                {
                    ...
                    {
                        distance = MathEx.DistanceToSegment(position, currentPoint, lastPoint);
                    ...
                    else
                    {
                        ...
                        foreach (var point in segment.generator.ForEachAdditionalSegmentPoint())
                        {
                            ...
                            distance = MathEx.DistanceToSegment(position, currentPoint, generatedPoint);
                            if (distance < maxDistance && distance < closestDistance)
                            {
                                closestDistance = distance;
                                result = segment;
        ...
        /// <summary>
        /// Finds the closest point on an edge (including Bezier polylines) suitable for inserting a new vertex,
        /// not too close to existing corners.
        /// </summary>
        public bool TryFindEdgeInsertPoint(float2 position, float maxEdgeDistance, float minDistFromVertex,
            out Segment hostSegment, out float2 closestOnEdge)
        {
            Segment? bestSeg = null;
            var bestPt = default(float2);
            ...
            for (var i = 0; i < shapesCount; i++)
            {
                ...
                for (var j = 0; j < segmentsCount; j++)
                {
                    ...
                    if (segment.generator.type == SegmentGeneratorType.Linear)
                    {
                        UpdateBestEdgeInsertCandidate(position, maxEdgeDistance, minDistFromVertex,
                            segment.position, lastPoint, segment, ref bestD, ref bestSeg, ref bestPt);
                    }
                    else
                    {
                        var currentPoint = segment.position;
                        foreach (var point in segment.generator.ForEachAdditionalSegmentPoint())
                        {
                            var generatedPoint = point;
                            UpdateBestEdgeInsertCandidate(position, maxEdgeDistance, minDistFromVertex,
                                currentPoint, generatedPoint, segment, ref bestD, ref bestSeg, ref bestPt);
                            currentPoint = generatedPoint;
                        }

                        UpdateBestEdgeInsertCandidate(position, maxEdgeDistance, minDistFromVertex,
                            currentPoint, lastPoint, segment, ref bestD, ref bestSeg, ref bestPt);
                    }
                }
            }

            hostSegment = bestSeg!;
            closestOnEdge = bestPt;
            return bestSeg != null;
        }

        static void UpdateBestEdgeInsertCandidate(float2 position, float maxEdgeDistance, float minDistFromVertex,
            float2 a, float2 b, Segment segment, ref float bestD, ref Segment? hostSegment, ref float2 closestOnEdge)
        {
            var nearest = MathEx.FindNearestPointOnLine(position, a, b);
            var d = math.distance(position, nearest);
            if (d >= maxEdgeDistance || d >= bestD)
                return;
            if (math.distance(nearest, a) < minDistFromVertex || math.distance(nearest, b) < minDistFromVertex)
                return;
            bestD = d;
            hostSegment = segment;
            closestOnEdge = nearest;
        }
