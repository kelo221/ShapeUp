		...
		if line.begins_with("}"):
			if brush:
				...
				if ent.properties["classname"] == "func_group" and ent.properties.has("_tb_type"):
					...
					elif not ent.brushes.is_empty():
						# Clipboard / snippet with only func_group (no worldspawn): brushes were dropped before.
						var ws := _EntityData.new()
						ws.properties = { "classname": "worldspawn", "mapversion": "220" }
						ws.brushes.append_array(ent.brushes)
						entities_data.append(ws)
